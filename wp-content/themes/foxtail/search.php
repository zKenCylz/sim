...
<main class="main main-archive main-search">
	<div class="container">
		<div class="wrap-bg">
			<div class="row">
                ...
                <section class="content col-md-8 col-sm-8 col-xs-12" role="main">
                    ...
                    <?php get_search_form(); ?>
                    <h1 class="archive-title">Kết quả tìm kiếm cho: <?php echo get_search_query() ?></h1>
                    <div class="posts-with-thumbnail posts-standard">
                        <?php
                        $sim_query = $wp_query->query;
                        if ($sim_query['post_type'] == "simso") {
                            ...
                            <form action="<?php echo $permalink; ?>" method="GET" role="form" class="form-inline">
                                <?php
                                if (isset($_GET['s'])) {
                                 echo '<input type="hidden" name="s" id="input" class="form-control" value="'.$s.'">';
                             }
                             if (isset($_GET['post_type'])) {
                                 echo '<input type="hidden" name="post_type" id="input" class="form-control" value="'.$post_type.'">';
                             }
                             ?>
                             <div class="form-group">
                                ...
                                <select name="sim_mang_di_dong" id="inputSim_mang_di_dong" class="form-control" required="required">
                                    <option value="all" <?php if ($sim_mang_di_dong=='all') {echo "selected";} ?>>Mọi mạng</option>
                                    <?php
                                    $args = array(
                                        'taxonomy' => 'mang_sim',
                                        'parent'   => 0,
